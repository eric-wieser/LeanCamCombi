...
#align_import LeanCamCombi.Kneser.kneser_ruzsa
...
lemma le_card_union_add_card_mulStab_union :
  ...
  have hl : ∀ a, (t \ s ∩ a • H).card = l a * (Ht.card - k a) := by sorry by_cases hkl :
  ...
  simp only [not_and_or, not_or, Classical.not_forall, not_ne_iff, not_imp] at hkl
...
lemma le_card_mul_add_card_mulStab_mul (hs : s.Nonempty) (ht : t.Nonempty) :
  ...
  rw [this]
  refine' (le_inf' ht.attach _ fun b _ => _).trans (le_card_sup_add_card_mul_stab_sup _)
  rw [← hstcard b]
  refine'
  ...
  refine'
    smul_finset_subset_iff.2
      ((inter_eq_left_iff_subset _ _).1 <| eq_of_subset_of_card_le (inter_subset_left _ _) _)
  rw [← ht']
  refine'
    Nat.find_min' _
...
lemma mul_kneser' (s t : Finset α) :
  ...
  · simp
  refine'
    (le_card_mul_add_card_mul_stab_mul (hs.mul (hs.mul ht).mulStab) <|
