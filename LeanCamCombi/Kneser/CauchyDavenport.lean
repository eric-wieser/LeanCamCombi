...
#align_import LeanCamCombi.Kneser.cauchy_davenport

/-!
# The Cauchy-Davenport lemma

This file proves a generalisation of the Cauchy-Davenport lemma to arbitrary groups.

## Main declarations

* `finset.min_le_card_mul`: A generalisation of the Cauchy-Davenport lemma to arbitrary groups.
* `monoid.is_torsion_free.card_add_card_sub_one_le_card_mul`: The Cauchy-Davenport lemma in
  torsion-free groups.
* `zmod.min_le_card_add`: The Cauchy-Davenport lemma.
* `finset.card_add_card_sub_one_le_card_mul`: The Cauchy-Davenport lemma in linear ordered
  cancellative semigroups.

## References

* Matt DeVos, *On a generalization of the Cauchy-Davenport lemma*
...
/-- The relation we induct along in the proof of Cauchy-Davenport lemma. `(s₁, t₁) < (s₂, t₂)` iff
* `|s₁ * t₁| < |s₂ * t₂|`
* or `|s₁ * t₁| = |s₂ * t₂|` and `|s₂| + |t₂| < |s₁| + |t₁|`
* or `|s₁ * t₁| = |s₂ * t₂|` and `|s₁| + |t₁| = |s₂| + |t₂|` and `|s₁| < |s₂|`. -/
@[to_additive
      "The relation we induct along in the proof of Cauchy-Davenport lemma.\n`(s₁, t₁) < (s₂, t₂)` iff\n* `|s₁ + t₁| < |s₂ + t₂|`\n* or `|s₁ + t₁| = |s₂ + t₂|` and `|s₂| + |t₂| < |s₁| + |t₁|`\n* or `|s₁ + t₁| = |s₂ + t₂|` and `|s₁| + |t₁| = |s₂| + |t₂|` and `|s₁| < |s₂|`."]
    ...
    DevosMulRel x y ↔
      (x.1 * x.2).card < (y.1 * y.2).card ∨
        (x.1 * x.2).card = (y.1 * y.2).card ∧ y.1.card + y.2.card < x.1.card + x.2.card ∨
          (x.1 * x.2).card = (y.1 * y.2).card ∧
            x.1.card + x.2.card = y.1.card + y.2.card ∧ x.1.card < y.1.card := by simp [devos_mul_rel, Prod.lex_iff, and_or_left]
...
lemma wellFoundedOn_devosMulRel :
  ...
  refine'
    is_well_founded.wf.on_fun.well_founded_on.prod_lex_of_well_founded_on_fiber fun n =>
      Set.WellFoundedOn.prod_lex_of_wellFoundedOn_fiber _ fun n =>
        is_well_founded.wf.on_fun.well_founded_on
  exact
    is_well_founded.wf.on_fun.well_founded_on.mono' fun x hx y hy =>
...
/-- A generalisation of the **Cauchy-Davenport lemma** to arbitrary groups. -/
@[to_additive "A generalisation of the **Cauchy-Davenport lemma** to arbitrary groups."]
lemma Finset.min_le_card_mul (hs : s.Nonempty) (ht : t.Nonempty) :
  ...
  ·
    simpa [← mul_inv_rev, add_comm] using
      ih _ _ ht.inv hs.inv
        (devosMulRel_iff.2 <| Or.inr <| Or.inr <| by simpa [← mul_inv_rev, add_comm])
  ...
  · have hS : (zpowers g : Set α) ⊆ a⁻¹ • s := by
      refine'
    ...
    exact
      Or.inl
        ((min_order_le_nat_card (zpowers_ne_bot.2 hg) <|
...
/-- The **Cauchy-Davenport lemma** for torsion-free groups. -/
@[to_additive "The **Cauchy-Davenport lemma** for torsion-free groups."]
lemma Monoid.IsTorsionFree.card_add_card_sub_one_le_card_mul (h : IsTorsionFree α)
  ...
  simpa only [h.min_order, min_eq_right, le_top, Nat.cast_le] using Finset.min_le_card_mul hs ht
...
/-- The **Cauchy-Davenport lemma** for linearly ordered cancellative semigroups. -/
@[to_additive
      "The **Cauchy-Davenport lemma** for linearly ordered additive cancellative\nsemigroups."]
lemma Finset.card_add_card_sub_one_le_card_mul [LinearOrder α] [CancelSemigroup α]
  ...
  refine'
    eq_singleton_iff_unique_mem.2
      ⟨mem_inter.2
          ⟨mul_mem_mul (max'_mem _ _) <| mem_singleton_self _,
            mul_mem_mul (mem_singleton_self _) <| min'_mem _ _⟩,
        _⟩
