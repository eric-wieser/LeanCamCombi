...
/-- A version of `Set.pi` for Sublattices. Given an index set `I` and a family of submodules
`s : Π i, Sublattice f i`, `pi I s` is the Sublattice of dependent functions `f : Π i, f i` such that
`f i` belongs to `pi I s` whenever `i ∈ I`. -/
def pi (I : Set η) (L : ∀ i, Sublattice (f i)) : Sublattice (∀ i, f i) where
  carrier := I.pi λ i ↦ (s i).carrier
  one_mem' i _ := (s i).one_mem
  mul_mem' hp hq i hI := (s i).mul_mem (hp i hI) (hq i hI)

lemma coe_pi (I : Set η) (L : ∀ i, Sublattice (f i)) :
    (pi I L : Set (∀ i, f i)) = Set.pi I λ i ↦ (L i : Set (f i)) :=
  rfl

lemma mem_pi (I : Set η) {L : ∀ i, Sublattice (f i)} {p : ∀ i, f i} :
    p ∈ pi I L ↔ ∀ i : η, i ∈ I → p i ∈ L i :=
  Iff.rfl

lemma pi_top (I : Set η) : (pi I λ i ↦ (⊤ : Sublattice (f i))) = ⊤ :=
  ext λ a ↦ by simp [mem_pi]

lemma pi_empty (L : ∀ i, Sublattice (f i)) : pi ∅ L = ⊤ :=
  ext λ a ↦ by simp [mem_pi]

lemma pi_bot : (pi Set.univ λ i ↦ (⊥ : Sublattice (f i))) = ⊥ :=
  (eq_bot_iff_forall _).mpr λ p hp ↦ by
    simp only [mem_pi, mem_bot] at *
    ext j
    exact hp j trivial

lemma le_pi_iff {I : Set η} {L : ∀ i, Sublattice (f i)} {J : Sublattice (∀ i, f i)} :
    J ≤ pi I L ↔ ∀ i : η, i ∈ I → map (Pi.evalLatticeHom f i) J ≤ L i := by
  constructor
  · intro h i hi
    rintro _ ⟨a, hx, rfl⟩
    exact (h hx) _ hi
...
lemma mulSingle_mem_pi [DecidableEq η] {I : Set η} {L : ∀ i, Sublattice (f i)} (i : η) (a : f i) :
    Pi.mulSingle i a ∈ pi I L ↔ i ∈ I → a ∈ L i := by
  constructor
  · intro h hi
    simpa using h i hi
  · intro h j hj
...
lemma pi_eq_bot_iff (L : ∀ i, Sublattice (f i)) : pi Set.univ L = ⊥ ↔ ∀ i, L i = ⊥ := by
  classical
    simp only [eq_bot_iff_forall]
    constructor
    · intro h i a hx
      have : LatticeHom.single f i a = 1 :=
        h (LatticeHom.single f i a) ((mulSingle_mem_pi i a).mpr λ _ ↦ hx)
      simpa using congr_fun this i
    · exact λ h a hx ↦ funext λ i ↦ h _ _ (hx i trivial)
...
variable {β : Type*} {P : Type*} [Group β] [Group P] (L : Sublattice α)
...
lemma range_eq_map (f : LatticeHom α β) : f.range = (⊤ : Sublattice α).map f := by ext; simp
...
homomorphism `LatticeHom α β`. -/
      "The canonical surjective `AddGroup` homomorphism `α →+ f(α)` induced by a lattice
      homomorphism `α →+ β`."]
def rangeRestrict (f : LatticeHom α β) : α →* f.range :=
...
lemma range_top_iff_surjective {β} [Group β] {f : LatticeHom α β} :
...
lemma range_top_of_surjective {β} [Group β] (f : LatticeHom α β) (hf : Surjective f) :
...
lemma sublatticeOf_range_eq_of_le {G₁ G₂ : Type*} [Group G₁] [Group G₂] {L : Sublattice G₂}
    (f : G₁ →* G₂) (h : f.range ≤ L) :
...
lemma prodMap_comap_prod {G' : Type*} {N' : Type*} [Group G'] [Group N'] (f : LatticeHom α β)
    (g : G' →* N') (S : Sublattice β) (S' : Sublattice N') :
    (S.prod S').comap (prodMap f g) = (S.comap f).prod (S'.comap g) :=
  SetLike.coe_injective $ Set.preimage_prod_map_prod f g _ _

lemma ker_prodMap {G' : Type*} {N' : Type*} [Group G'] [Group N'] (f : LatticeHom α β) (g : G' →* N') :
...
variable {β : Type*} [Group β] (L : Sublattice α)
...
variable {β : Type*} [Group β] (f : LatticeHom α β)
